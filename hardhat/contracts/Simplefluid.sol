...
contract Simplefluid is AutomationCompatibleInterface {
    ...
    address public constant _flowScheduler =
        0xF18825d412C061aEfEFB4dF46a1c077636dA50bf;
    address public constant _token = 0x30a6933Ca9230361972E413a15dC8114c952414e; //fDAIx
            ...
            for (uint j = 0; j < pair.length; j++) {
                ...
                if (
                    isAutomated[
                        keccak256(
                            abi.encodePacked(
                                automatedStreams[automatedSenders[i]],
                                pair[j]
                            )
                        )
                    ]
                ...
                IFlowScheduler.FlowSchedule memory getFlow = getScheduledStream(
                    automatedSenders[i],
                    pair[j]
                );

                if (
                    block.timestamp > getFlow.startDate &&
                    block.timestamp < getFlow.startDate + getFlow.startMaxDelay
                ) {
                    upkeepNeeded = true;
                }
     ...
     */
    function getScheduledStream(
        address sender,
        address receiver
    ) public view returns (IFlowScheduler.FlowSchedule memory) {
        return flowScheduler.getFlowSchedule(_token, sender, receiver);
    }

    //function to start the scheduled stream
    function startScheduledStream(
        address sender,
        address receiver
    ) public returns (bool) {
