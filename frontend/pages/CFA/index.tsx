      ...
      {!update && showAlert && (
        <div className={`alert alert-info shadow-lg ${styles.alert}`}>
          <div>
            ...
            <span>To enable multi stream, you need to provide permission to smart contract.</span>
          </div>
        </div>
      )}
      <div className={styles.page}>
        ...
        <div>
          {currentAccounts.map((_, index) => {
            return <CfaPage key={index} id={index + 1} update={updateReceiverAddress} receiver={receiver} />;
          })}
        </div>
        <div className={styles.fields}>
          <div className={styles.flowRate}>
            <label>Flow Rate</label>
            <input type="text" placeholder="Flow Rate/second" className={`${styles.flow}`} ref={flowRateRef} />
          </div>
          <div>
            <label>Super token</label>
            <input type="text" placeholder="ETHx" className={`input input-bordered w-full max-w-xs ${styles.token}`} disabled />
          </div>
        </div>
        {update !== "true" ? (
          <div style={{ paddingBottom: "3%", marginTop: "3%" }}>
            <button className={`btn btn-info ${styles.button}`} onClick={incrementHandler}>
              <i className="fa-regular fa-user-plus"></i>&nbsp;Add wallet address
            </button>
            <button className={`btn btn-warning ${styles.send} ${loading ? "loading" : ""}`} onClick={streamHandler}>
              {loading ? "Starting" : "Start streaming"}
            </button>
          </div>
        ) : (
          <div style={{ paddingBottom: "3%", marginTop: "3%" }}>
            <button className={`btn btn-warning ${styles.modify} ${loading ? "loading" : ""}`} onClick={streamHandler}>
              {loading ? "Modifying" : "Modify Stream"}
            </button>
          </div>
        )}
      </div>
